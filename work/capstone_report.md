...
## 0. Abstract
> Content decay quietly erodes organic traffic, but identifying which pages will drop next is notoriously difficult when managing thousands of URLs. Using an anonymized dataset of 17,980 pages from FlyRank, we developed a Random Forest model to predict traffic declines over a 90-day window. By learning from features covering traffic, engagement, and metadata, the model achieved a 74% precision (Precision@50) on the top 50 highest-risk pages, significantly outperforming a human-defined baseline rule (24%). This paper provides a directional decision-support framework for SEO managers to prioritize their content refresh queues efficiently.
